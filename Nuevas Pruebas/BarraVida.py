...
def barra_hp(pantalla,x,y, hp):
    largo = 200
    ancho = 25
    #100 es la vida, por el largo, es decir porcentaje de la barra
    calculo_barra = int((jugador.hp/100)*largo)
    #Aqui se especifica como es el borde y el rectangulo0
    borde = pygame.Rect(x,y, largo, ancho)
    rectangulo = pygame.Rect(x,y, calculo_barra, ancho)
    #Aqui se dibuja
    pygame.draw.rect(pantalla, AZUL_2, borde, 3)
    pygame.draw.rect(pantalla, AZUL_OSCURO, rectangulo)
    pantalla.blit(pygame.transform.scale(jugador.image(25,25)), (545,15))
    warning = pygame.image.load("Nsprites/Warning.png").convert
    if jugador.hp <0:
        jugador.hp =0
    if jugador.hp <30:
...
while ejecutando:
    ...
    colision_meteorito1=pygame.sprite.spritecollide(jugador, enemigos_1, False, pygame.sprite.collide_circle)
    if colision_meteorito1:
        if puntuacion >=0:
            puntuacion -=10
            if puntuacion <0:
                puntuacion=0
            
    colision_meteorito2=pygame.sprite.spritecollide(jugador, enemigos_2, False, pygame.sprite.collide_circle)
    if colision_meteorito2:
        if puntuacion >=0:
            puntuacion -=10
            if puntuacion <0:
                puntuacion=0
    
    colision_meteorito3=pygame.sprite.spritecollide(jugador, enemigos_3, False, pygame.sprite.collide_circle)
    if colision_meteorito3:
        if puntuacion >=0:
            puntuacion -=10
            if puntuacion <0:
                puntuacion=0
    
    colision_meteorito4=pygame.sprite.spritecollide(jugador, enemigos_4, False, pygame.sprite.collide_circle)
    if colision_meteorito4:
        if puntuacion >=0:
            puntuacion -=10
            if puntuacion <0:
                puntuacion=0
    
    colision_disparos_1= pygame.sprite.groupcollide(enemigos_1, balas, False, True, pygame.sprite.collide_circle)
    if colision_disparos_1:
        puntuacion +=10
    
    colision_disparos_2= pygame.sprite.groupcollide(enemigos_2, balas, False, True, pygame.sprite.collide_circle)
    if colision_disparos_2:
        puntuacion +=10
    
    colision_disparos_3= pygame.sprite.groupcollide(enemigos_3, balas, False, True, pygame.sprite.collide_circle)
    if colision_disparos_1:
        puntuacion +=10

    
    colision_disparos_4= pygame.sprite.groupcollide(enemigos_4, balas, False, True, pygame.sprite.collide_circle)
    if colision_disparos_1:
        puntuacion +=10
    ...
    muestra_texto(pantalla,arial,str(puntuacion).zfill(7), MORADO_CLARO, 40, 680, 60)
    #En el argumento se especifica el jugador.hp para que haga el calculo de procentaje en torno a la vida del jugador
    #la barra seria un 100*100, donde si se impacta un enemigo que quita 15, quitaria el 15% y ese es el porcen taje de barra que va a ir retirando
